...
namespace HermesChat_TeamA.Hubs
{
    ...
    public class SyncHub : Hub
    {
        ...
        public async Task SendToParticularUser(string user, string receiver, string message)
        {
            ...
            _dataAccessService.CreateMessage(Context.User.Identity.Name, receiver, message);
        ...
        public async Task SendMessageToGroup(string user, string groupName, string message)
        {
            ...
            await Clients.Group(groupName).SendAsync("ReceiveMessage", Context.User.Identity.Name ?? "anonymous", groupName, message);
            // trūksta

            _dataAccessService.CreateMessage(Context.User.Identity.Name, groupName, message);
        }
